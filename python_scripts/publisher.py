...
# Chat configuration
CHAT_USER_ID = os.getenv("CHAT_USER_ID", "fredy")
CHAT_USER_NAME = os.getenv("CHAT_USER_NAME", "Fredy")
...
class ChatPublisher:
    ...
    def on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            # Subscribe to general chat topic and user-specific topic for status updates
    ...
    def handle_status_update(self, data):
        ...
        if message_id in self.pending_messages:
            self.pending_messages[message_id]["status"] = status
            self.pending_messages[message_id]["last_update"] = datetime.now()
    ...
    def get_pending_messages_status(self):
        """Get status of pending messages."""
    ...
    def display_help(self):
        ...
        print("status                    - Show pending message status")
        ...
        while self.running:
            try:
                ...
                if command == "send" and len(parts) >= 3:
                    ...
                    pending = self.get_pending_messages_status()
                    if pending:
                        print(f"\n📊 Pending messages ({len(pending)}):")
                        for msg_id, details in pending.items():
                            print(f"  {msg_id[:8]}... - {details['status']} to {details['receiver_id']}")
                    else:
    ...
    def run(self):
        """Main method to run the chat publisher."""
        # 1. Establish database connection
        self.db_conn = self.get_db_connection()
        self.setup_database(self.db_conn)

        # 2. Setup MQTT Client
        self.client = mqtt.Client(client_id=f"chat-publisher-{CHAT_USER_ID}")
